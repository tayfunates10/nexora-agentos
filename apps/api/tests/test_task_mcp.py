...
def test_task_adapter_routes_run_scoped_operations():
    ...
    created = asyncio.run(adapter.call_tool_for_context(
        context(), "child.create", {"title": "Follow up"}, 1.0
    ))
    verified = asyncio.run(adapter.call_tool_for_context(
...
def test_task_adapter_rejects_custom_agent_context():
    ...
    with pytest.raises(McpAdapterError) as raised:
        asyncio.run(adapter.call_tool_for_context(
            context("custom"), "child.create", {"title": "No"}, 1.0
        ))
...
def test_task_adapter_preserves_stable_repository_error_code():
    ...
    with pytest.raises(McpAdapterError) as raised:
        asyncio.run(adapter.call_tool_for_context(
            context(), "task.verify",
            {"task_id": "00000000-0000-0000-0000-000000000001"}, 1.0
        ))
