...
import { beginLogin, cookieNames, cookieOptions, readConfig } from "../../../lib/auth-core";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (request.headers.get("origin") !== config.origin) return new NextResponse("Forbidden", { status: 403 });
