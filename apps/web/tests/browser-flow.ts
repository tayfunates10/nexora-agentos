...
try {
  ...
  await page.goto(origin + "/workspaces");
  await expect(page).toHaveURL(origin + "/login");
  console.log("Browser flow passed: OIDC sign-in, create, rename, membership, CSRF, agent and run console, tool policy and approval decisions, knowledge ingestion, spend and budget console, responsive layouts, role UI and logout replay rejection");
} finally {
