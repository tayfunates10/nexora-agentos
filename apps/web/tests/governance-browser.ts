...
export async function checkGovernance(
  ...
  await sidebarLink(page, t("navigation.tools")).click();
  await expect(page.getByRole("heading", { name: t("tools.title") })).toBeVisible();
  await expect(page.getByRole("heading", { name: t("tools.emptyTitle") })).toBeVisible();
  ...
  await page.reload();
  await expect(page.getByText(t("tools.policy.none"))).toBeVisible();
  await expect(page.getByText(t("tools.forcesApproval", { link: t("tools.forcesApprovalLink") }))).toBeVisible();

  await page.getByLabel(t("tools.decisionLabel")).first().selectOption("require_approval");
  await page.getByLabel(t("tools.reasonLabel")).first().fill("Handbook search needs review while we pilot it.");
  await page.getByRole("button", { name: t("tools.savePolicy") }).first().click();
  await expect(page.getByRole("status")).toContainText(t("tools.saved"));
  ...
  await page.goto(detailUrl);
  await sidebarLink(page, t("navigation.approvals")).click();
  await expect(page.getByRole("heading", { name: t("approvals.emptyPendingTitle") })).toBeVisible();
  ...
  await page.getByRole("button", { name: t("approvals.reject") }).click();
  await expect(page.getByRole("status")).toContainText(t("approvals.rejectedNotice"));
  await expect(page.getByRole("heading", { name: t("approvals.emptyPendingTitle") })).toBeVisible();
  await page.getByRole("link", { name: t("approvals.status.rejected"), exact: true }).click();
  await expect(page.getByRole("heading", { name: "delete-record" })).toBeVisible();
  await expect(page.getByRole("button", { name: t("approvals.reject") })).toHaveCount(0);
  await expect(page.getByText(t("approvals.closedNotice"))).toBeVisible();
  ...
  await expect(page.getByText(t("approvals.error.gone"))).toBeVisible();
  ...
  for (const width of [1440, 390]) {
    ...
    await page.screenshot({ path: `/tmp/nexora-approvals-${width}.png`, fullPage: true, animations: "disabled" });
  }
  ...
  await expect(page.getByRole("heading", {
    name: t("errors.notFoundOrDenied", { area: t("approvals.area") }),
  })).toBeVisible();
  await page.goto(`${detailUrl}/tools`);
  await expect(page.getByText(t("tools.readOnlyNotice"))).toBeVisible();
  await expect(page.getByRole("button", { name: t("tools.savePolicy") })).toHaveCount(0);
