...
export async function checkSpend(
  ...
  // 26 agent-run calls at 2.5 units each plus one judge call at 0.5 units.
  let newestId = "";
  for (let i = 0; i < 26; i++) {
    const id = randomUUID();
    if (i === 25) newestId = id;
  ...
  await page.reload();
  await expect(page.locator(".spend-panel")).toContainText("65.50");
  await expect(page.locator(".spend-panel")).toContainText("65,500,000 micros");
  ...
  await expect(page.locator(".spend-panel")).toContainText("34.75");
  await expect(page.getByRole("meter")).toHaveAttribute("aria-valuenow", "65500000");
  ...
  await page.goto(root + "?category=embeddings");
  await expect(page.getByRole("heading", { name: "Invalid spend link" })).toBeVisible();
